...
class AgentManager {
  ...
  async startAgent(agentId) {
    try {
      ...
      // 获取 Agent 配置以确定端口
      const agentConfig = await this.getAgentInfo(agentId);
      const port = agentConfig.config?.network?.port || 18791;
      ...
      cli.debug(`Agent "${agentId}" 启动成功，PID: ${openclawProcess.pid}, 端口: ${port}`);
      return true;
  ...
  async waitForGatewayStart(port, timeout = 10000) {
    ...
    throw new Error(`Gateway 在 ${timeout}ms 内未启动`);
      ...
      const env = {
        ...process.env,
        OPENCLAW_CONFIG_PATH: openclawConfigPath,
        OPENCLAW_STATE_DIR: path.join(agentPath, "data"),
      };

      const openclawProcess = spawn("openclaw", ["gateway"], {
        cwd: agentPath,
        env: env,
        stdio: ["ignore", "pipe", "pipe"],
        detached: true,
      ...
      // 保存进程引用
      this.runningAgents.set(agentId, openclawProcess);
  ...
  async stopAllAgents() {
    try {
      ...
      const runningAgents = agents.filter((agent) => agent.status === "运行中");
