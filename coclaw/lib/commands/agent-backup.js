...
async function chatWithAgent(agentManager, agentId) {
  ...
  const configPath = agentInfo.openclaw?.configPath || 
                     path.join(agentInfo.agentPath, 'openclaw.json');
  const stateDir = agentInfo.openclaw?.dataDir || 
                   path.join(agentInfo.agentPath, 'data');
  ...
  // 这里会调用 openclaw chat 命令
  // 暂时先提示
  cli.warn("聊天功能开发中，请稍后使用");
  cli.info("目前请使用 openclaw 直接与 Agent 交互");
}
...
async function configureRelation(agentManager, agentId) {
  ...
  if (otherAgents.length === 0) {
    ...
    cli.info("请先创建其他 Agent");
    return;
  }

  // 这里会显示关系配置界面
  // 暂时先提示
  cli.warn("关系配置功能开发中，请稍后使用");
  cli.info("可用 Agent 列表:");

  otherAgents.forEach((agent) => {
    cli.info(`  - ${agent.id} (${agent.name})`);
  });
}
